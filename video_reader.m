...
%書き込みモード用ラジオボタンの設定
Writing_ID = uicontrol('style','radiobutton','String','Writing');
Writing_position = round([0.18*fig_position(1,3) 0.12*fig_position(1,4) 0.22*fig_position(1,3) 0.05*fig_position(1,4)]); % [角横、縦、幅、高さ]
set(Writing_ID, 'position', Writing_position, 'HorizontalAlignment','center','FontName','Arial','FontSize',10,'FontWeight','bold',...
    'value',0);

%ロード表示用ラジオボタンの設定
Loading_ID = uicontrol('style','radiobutton','String','Loading');
Loading_position = round([0.42*fig_position(1,3) 0.12*fig_position(1,4) 0.22*fig_position(1,3) 0.05*fig_position(1,4)]); % [角横、縦、幅、高さ]
set(Loading_ID, 'position', Loading_position, 'HorizontalAlignment','center','FontName','Arial','FontSize',10,'FontWeight','bold',...
    'value',0);

%ロード表示用ラジオボタンの設定
Done_ID = uicontrol('style','togglebutton','String','Done');
Done_position = round([0.67*fig_position(1,3) 0.12*fig_position(1,4) 0.22*fig_position(1,3) 0.05*fig_position(1,4)]); % [角横、縦、幅、高さ]
set(Done_ID, 'position', Done_position, 'HorizontalAlignment','center','FontName','Arial','FontSize',10,'FontWeight','bold',...
    'value',0);
